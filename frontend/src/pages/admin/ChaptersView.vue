  ...
  <AdminLayout title="Kelola Episode">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
      <div>
        ...
        <p class="text-gray-500 text-sm mt-1">Klik episode untuk kelola halaman. Tambah, edit, dan hapus chapter komik ini.</p>
      </div>
        ...
        <table class="w-full text-sm text-left">
          ...
          <tbody class="divide-y divide-gray-100">
            ...
            <tr
              ...
              @click="router.push(`/admin/comics/${comicId}/chapters/${ch.id}/pages`)"
            >
              <td class="px-6 py-4">
              ...
              <td class="px-6 py-4 text-gray-500">{{ ch.pages_count ?? '-' }} halaman</td>
              <td class="px-6 py-4 text-gray-500 text-xs">{{ formatDate(ch.created_at) }}</td>
              <td class="px-6 py-4 text-right" @click.stop>
                <div class="inline-flex items-center justify-end gap-2">
                  <button @click="openModal('edit', ch)" title="Edit Episode" class="w-8 h-8 rounded-lg border border-gray-200 hover:bg-blue-50 hover:border-blue-200 hover:text-blue-600 flex items-center justify-center text-gray-500 transition-colors">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
                  </button>
                  <button @click="confirmDelete(ch)" title="Hapus Episode" class="w-8 h-8 rounded-lg border border-gray-200 hover:bg-red-50 hover:border-red-200 hover:text-red-600 flex items-center justify-center text-gray-500 transition-colors">
      ...
      <div v-if="showFormModal" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-gray-900/60 backdrop-blur-sm">
        <div class="bg-white rounded-xl shadow-2xl w-full max-w-md">
          ...
          <form @submit.prevent="submitForm" class="p-6 space-y-5">
            ...
            <div class="space-y-1.5">
              <label class="text-sm font-semibold text-gray-700">Judul Episode <span class="text-gray-400 font-normal">(Opsional)</span></label>
              <input
                v-model="form.title"
                type="text"
  ...
  try {
    if (modalMode.value === 'create') {
      const res = await fetch(`http://localhost:8000/api/v1/comics/${comicId}/chapters`, {
        ...
        body: JSON.stringify({ chapter_number: form.chapter_number, title: form.title || null })
      })
      const data = await res.json()
